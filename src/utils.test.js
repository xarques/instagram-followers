...
const expectedCroppedHourTimeSeriesEvents = [
  ...
  { index: "1h-427462", data: { value: 70 } },
  { index: "1h-427463", data: { value: 102 } }
];
...
test("time Series", () => {
  ...
  expect(JSON.stringify(events)).toEqual(
    JSON.stringify(expectedTimeSeriesEvents)
  );
});
...
test("time Series divided into days", () => {
  ...
  expect(JSON.stringify(events)).toEqual(
    JSON.stringify(expectedIndexTimeSeriesEvents)
  );
});
...
test("time Series cropped into 1 day", () => {
  ...
  const croppedSeries = buildCroppedTimeSeries(
    series,
    "1d",
    undefined,
    new Date("2018-10-06T23:59:59.000Z")
  );
  const events = croppedSeries.collection().eventList();
  expect(JSON.stringify(events)).toEqual(
    JSON.stringify(expectedCroppedHourTimeSeriesEvents)
  );
});
...
test("time Series cropped into 1 week", () => {
  ...
  const croppedSeries = buildCroppedTimeSeries(
    ...
    "1w",
    undefined,
    new Date("2018-10-08T11:00:00.000Z")
  );
  const events = croppedSeries.collection().eventList();
  expect(JSON.stringify(events)).toEqual(
    JSON.stringify(expectedCroppedDayTimeSeriesEvents)
  );
});
...
test("delta time Series", () => {
  ...
  expect(series.columns()).toEqual(["value"]);
  expect(JSON.stringify(events)).toEqual(
    JSON.stringify(expectedDeltaTimeSeriesEvents)
  );
});
